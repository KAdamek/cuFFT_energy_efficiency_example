...
for HARM in 8 #27 81 177147 1594323 125 3125 15625 78125 390625 1953125 121 
#for LENGTH in 48 96 192 384 768 1536 3072 6144 12288 24576 49152 98304 196608 393216 786432 1572864
do
rm timing-${ID}-${PREC}-${TYPE}-${HARM}.txt

#run the logger on the energy profiling
	nvidia-smi --query-gpu=timestamp,power.draw,clocks.current.sm,clocks.current.memory --format=csv,noheader,nounits -i $CARD -f nvidiasmi-${ID}-${PREC}-${TYPE}-${HARM}.txt -lms 10 &
	PID=$(echo $!)
...
#	nvprof --print-gpu-trace -u ms --csv ../cuFFT_benchmark.exe ${LENGTH} 0 0 -2 200 ${PREC} ${TYPE} ${CARD} 2>out.nvprof
	...
	sleep 5
	echo ${TIMESTAMP}"," ${START_TIME}"," ${END_TIME}"," ${END_PLUS_TIME}"," ${HARM}>> timing-${ID}-${PREC}-${TYPE}-${HARM}.txt
