...
do
rm timing-${ID}-${PREC}-${TYPE}-${HARM}-${CORE_CLOCK}-${NRUNS}.txt

#run the logger on the energy profiling
	nvidia-smi --query-gpu=timestamp,power.draw,clocks.current.sm,clocks.current.memory --format=csv,noheader,nounits -i $CARD -f nvidiasmi-${ID}-${PREC}-${TYPE}-${HARM}-${CORE_CLOCK}-${NRUNS}.txt -lms 10 &
	PID=$(echo $!)
...
#	nvprof --print-gpu-trace -u ms --csv ../cuFFT_benchmark.exe ${LENGTH} 0 0 -2 200 ${PREC} ${TYPE} ${CARD} 2>out.nvprof
	nvprof --print-gpu-trace -u ms --csv ../HRMS_benchmark.exe 500000 ${HARM} 1024 ${NRUNS} ${CORE_CLOCK} ${CARD} 2>out.nvprof
	...
	sleep 30
	echo ${TIMESTAMP}"," ${START_TIME}"," ${END_TIME}"," ${END_PLUS_TIME}"," ${HARM}>> timing-${ID}-${PREC}-${TYPE}-${HARM}-${CORE_CLOCK}-${NRUNS}.txt
